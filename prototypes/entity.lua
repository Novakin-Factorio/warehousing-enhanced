...
local ICONPATH = "__warehousing-enhanced__/graphics/icons/"
local ENTITYPATH = "__warehousing-enhanced__/graphics/entity/"
...
local function connectorSprite(shift, shiftshadow)
return
	{
		connector_main =
		{
...
local function chestAnim(img, shadow, shadowshift, width, height, chestanim, chestshift, chestanimsize)
return
	{
		layers = {
			{
...
local function createLogisticContainer(name, logistic_type)
	...
	p.robot_door = {
		...
		animation_sound = p.animation_sound,
		opened_duration = p.opened_duration,
		location_offset = name == "warehouse" and {1.0, -1.0} or nil,
	}
	p.animation = nil
	p.animation_sound = nil
	p.opened_duration = nil
...
local function createLinkedContainer(name)
	local p = table.deepcopy(data.raw["container"][name.."-basic"])
